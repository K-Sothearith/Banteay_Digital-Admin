...
function AlertModal({ alert, onClose, onSave }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-xs" role="presentation">
      <section className="w-full max-w-2xl overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl dark:border-[#1e3568] dark:bg-[#0c1733]" role="dialog" aria-modal="true" aria-labelledby="alert-modal-title">
        <header className="flex items-center justify-between border-b border-slate-100 px-6 py-4 dark:border-[#1e3568]/60">
          <div>
        ...
        <form onSubmit={submit} className="space-y-5 p-6">
          ...
          <footer className="flex justify-end gap-3 border-t border-slate-100 pt-4 dark:border-[#1e3568]/60">
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        <button type="button" onClick={() => setEditing(null)} className="rounded-xl bg-[#012475] px-4 py-2.5 text-sm font-bold text-white shadow-sm dark:bg-[#4b9efe] dark:text-[#070d1e]"><i className="fa-solid fa-plus mr-2" />New alert</button>
      </div>
      ...
      {!loading && visibleAlerts.length ? (
        <div className="grid gap-4 lg:grid-cols-2">
          {visibleAlerts.map((alert) => (
            <article key={alert.id} className="flex flex-col rounded-2xl border border-slate-200 bg-white p-5 dark:border-[#1e3568] dark:bg-[#0c1733]">
